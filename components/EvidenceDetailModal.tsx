...
export default function EvidenceDetailModal({ evidence, onClose }: EvidenceDetailModalProps) {
    if (!evidence.detalhes) return null;

    const { detalhes } = evidence;
            ...
            <div className="bg-white w-full max-w-2xl h-full overflow-y-auto shadow-2xl animate-slide-in-right">
                ...
                <div className="sticky top-0 bg-white/90 backdrop-blur border-b border-gray-100 px-8 py-6 flex justify-between items-start z-10">
                    <div>
                        ...
                        <h2 className="text-2xl font-bold text-gray-900 leading-tight">{evidence.titulo}</h2>
                    </div>
                ...
                <div className="p-8 space-y-12">
                    ...
                    <aside className="bg-pink-50/50 p-6 rounded-2xl border-l-4 border-brand-brown text-gray-700 italic leading-relaxed">
                        ...
                        <div dangerouslySetInnerHTML={{ __html: formatText(detalhes.resumo_aside) }} />
                    </aside>
                    ...
                    <section>
                        ...
                        <ul className="list-disc pl-5 space-y-2 text-gray-700">
                            {(detalhes.publico_alvo || []).map((item, i) => <li key={i} dangerouslySetInnerHTML={{ __html: item }} />)}
                        </ul>
                    </section>
                    ...
                    <section>
                        ...
                        <div className="space-y-4">
                            {(detalhes.metodologia_passos || []).map((step, i) => (
                    ...
                    <section className="bg-gray-50 p-6 rounded-2xl border border-gray-200">
                        ...
                        <div className="grid grid-cols-2 gap-4 mb-6">
                            {(detalhes.resultados_key || []).map((res, i) => (
                        ...
                        <div className="overflow-x-auto">
                            <table className="w-full text-sm text-left">
                                ...
                                <tbody className="divide-y divide-gray-100">
                                    {(detalhes.tabela_evidencias || []).map((row, i) => (
                    ...
                    <section>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="bg-green-50 p-4 rounded-xl border border-green-100">
                                ...
                                <p className="text-sm text-green-700">{detalhes.recomendacoes?.uso}</p>
                            </div>
                            <div className="bg-red-50 p-4 rounded-xl border border-red-100">
                                <div className="font-bold text-red-800 mb-1">Quando evitar</div>
                                <p className="text-sm text-red-700">{detalhes.recomendacoes?.evitar}</p>
                            </div>
                            <div className="bg-blue-50 p-4 rounded-xl border border-blue-100 md:col-span-2">
                                ...
                                <p className="text-sm text-blue-700">{detalhes.recomendacoes?.recursos}</p>
                            </div>
                        </div>
                    </section>
                </div>
